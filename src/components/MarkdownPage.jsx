...
export default function MarkdownPage({ markdown, page, eyebrow, title, subtitle }) {
  const slugCounts = {};
  ...
  const components = {
    ...
    a: ({ href, children }) => {
      if (href?.startsWith("#")) {
        return (
          <a href={`#/${page}/${href.slice(1)}`} className="text-violet-300 underline decoration-violet-500/40 underline-offset-3 hover:text-violet-100">
            {children}
          </a>
        );
      }
